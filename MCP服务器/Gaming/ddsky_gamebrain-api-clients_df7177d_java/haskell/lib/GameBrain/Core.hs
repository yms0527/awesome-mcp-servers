...
-- |
data GameBrainConfig = GameBrainConfig
...
-- | display the config
instance P.Show GameBrainConfig where
  show c =
    T.printf
...
-- | constructs a default GameBrainConfig
--
-- configHost:
--
-- @https://api.gamebrain.co/v1@
--
-- configUserAgent:
--
-- @"gamebrain/0.1.0.0"@
--
newConfig :: IO GameBrainConfig
newConfig = do
    logCxt <- initLogContext
    return $ GameBrainConfig
        { configHost = "https://api.gamebrain.co/v1"
        , configUserAgent = "gamebrain/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => GameBrainConfig -> auth -> GameBrainConfig
addAuthMethod config@GameBrainConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: GameBrainConfig -> IO GameBrainConfig
...
withStderrLogging :: GameBrainConfig -> IO GameBrainConfig
...
withNoLogging :: GameBrainConfig -> GameBrainConfig
...
data GameBrainRequest req contentType res accept = GameBrainRequest
  { rMethod  :: NH.Method   -- ^ Method of GameBrainRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of GameBrainRequest
  , rParams   :: Params -- ^ params of GameBrainRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (GameBrainRequest req contentType res accept) NH.Method
rMethodL f GameBrainRequest{..} = (\rMethod -> GameBrainRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (GameBrainRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f GameBrainRequest{..} = (\rUrlPath -> GameBrainRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (GameBrainRequest req contentType res accept) Params
rParamsL f GameBrainRequest{..} = (\rParams -> GameBrainRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (GameBrainRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f GameBrainRequest{..} = (\rAuthTypes -> GameBrainRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => GameBrainRequest req contentType res accept -> param -> GameBrainRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: GameBrainRequest req contentType res accept -> param -> GameBrainRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: GameBrainRequest req contentType res accept -> param -> GameBrainRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** GameBrainRequest Utils

_mkRequest :: NH.Method -- ^ Method
          -> [BCL.ByteString] -- ^ Endpoint
          -> GameBrainRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = GameBrainRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     GameBrainRequest req contentType res accept
  -> [NH.Header]
  -> GameBrainRequest req contentType res accept
...
removeHeader :: GameBrainRequest req contentType res accept -> [NH.HeaderName] -> GameBrainRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => GameBrainRequest req contentType res accept -> GameBrainRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => GameBrainRequest req contentType res accept -> GameBrainRequest req contentType res accept
...
addForm :: GameBrainRequest req contentType res accept -> WH.Form -> GameBrainRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: GameBrainRequest req contentType res accept -> NH.Part -> GameBrainRequest req contentType res accept
...
_setBodyBS :: GameBrainRequest req contentType res accept -> B.ByteString -> GameBrainRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: GameBrainRequest req contentType res accept -> BL.ByteString -> GameBrainRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => GameBrainRequest req contentType res accept -> P.Proxy authMethod -> GameBrainRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> GameBrainRequest req contentType res accept
    -> IO (GameBrainRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> GameBrainConfig
  -> IO (GameBrainRequest req contentType res accept)
_applyAuthMethods req config@(GameBrainConfig {configAuthMethods = as}) =
  foldlM go req as
  where
