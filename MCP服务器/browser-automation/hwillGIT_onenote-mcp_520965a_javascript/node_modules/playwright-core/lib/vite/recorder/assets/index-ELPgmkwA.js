...
`)?Zt(r,n):r.includes(`
`)?`
`+Zt(r,n):(i.endsWith(" ")?"":" ")+r,gh="flow",Fa="block",rl="quoted";function gl(i,n,r="flow",{indentAtStart:l,lineWidth:o=80,minContentWidth:u=20,onFold:f,onOverflow:h}={}){if(!o||o<0)return i;o<u&&(u=0);const p=Math.max(1+u,1+o-n.length);if(i.length<=p)return i;const g=[],x={};let S=o-n.length;typeof l=="number"&&(l>o-Math.max(2,u)?g.push(0):S=o-l);let T,E,O=!1,v=-1,w=-1,I=-1;r===Fa&&(v=Nd(i,v,n.length),v!==-1&&(S=v+p));for(let F;F=i[v+=1];){if(r===rl&&F==="\\"){switch(w=v,i[v+1]){case"x":v+=3;break;case"u":v+=5;break;case"U":v+=9;break;default:v+=1}I=v}if(F===`
`)r===Fa&&(v=Nd(i,v,n.length)),S=v+n.length+p,T=void 0;else{if(F===" "&&E&&E!==" "&&E!==`
`&&E!=="	"){const V=i[v+1];V&&V!==" "&&V!==`
`&&V!=="	"&&(T=v)}if(v>=S)if(T)g.push(T),S=T+p,T=void 0;else if(r===rl){for(;E===" "||E==="	";)E=F,F=i[v+=1],O=!0;const V=v>I+1?v-2:w-1;if(x[V])return i;g.push(V),x[V]=!0,S=V+p,T=void 0}else O=!0}E=F}if(O&&h&&h(),g.length===0)return i;f&&f();let P=i.slice(0,g[0]);for(let F=0;F<g.length;++F){const V=g[F],W=g[F+1]||i.length;V===0?P=`
${n}${i.slice(0,W)}`:(r===rl&&x[V]&&(P+=`${i[V]}\\`),P+=`
${n}${i.slice(V+1,W)}`)}return P}function Nd(i,n,r){let l=n,o=n+1,u=i[o];for(;u===" "||u==="	";)if(n<o+r)u=i[++n];else{do u=i[++n];while(u&&u!==`
...
`,p+=2;f+=u,r[p+2]===" "&&(f+="\\"),p+=1,h=p+1}break;default:p+=1}return f=h?f+r.slice(h):r,l?f:gl(f,u,rl,yl(n,!1))}function za(i,n){if(n.options.singleQuote===!1||n.implicitKey&&i.includes(`
`)||/[ \t]\n|\n[ \t]/.test(i))return _s(i,n);const r=n.indent||(vl(i)?"  ":""),l="'"+i.replace(/'/g,"''").replace(/\n+/g,`$&
${r}`)+"'";return n.implicitKey?l:gl(l,r,gh,yl(n,!1))}function xr(i,n){const{singleQuote:r}=n.options;let l;if(r===!1)l=_s;else{const o=i.includes('"'),u=i.includes("'");o&&!u?l=za:u&&!o?l=_s:l=r?za:_s}return l(i,n)}let Ua;try{Ua=new RegExp(`(^|(?<!
))
+(?!
...
`&&(E=E.slice(0,-1)),E=E.replace(Ua,`$&${g}`));let v=!1,w,I=-1;for(w=0;w<r.length;++w){const j=r[w];if(j===" ")v=!0;else if(j===`
`)I=w;else break}let P=r.substring(0,I<w?I+1:w);P&&(r=r.substring(P.length),P=P.replace(/\n+/g,`$&${g}`));let V=(x?"|":">")+(v?g?"2":"1":"")+S;if(i&&(V+=" "+h(i.replace(/ ?[\r\n]+/g," ")),o&&o()),x)return r=r.replace(/\n+/g,`$&${g}`),`${V}
${g}${P}${r}${E}`;r=r.replace(/\n+/g,`
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${g}`);const W=gl(`${P}${r}${E}`,g,Fa,yl(l,!0));return`${V}
...
`;for(let v=x;v<h;++v){let[w,I]=f[v];g+=w.length+I.length+1;const P=I[I.length-1]==="\r";if(P&&(I=I.slice(0,-1)),I&&w.length<p){const V=`Block scalar lines must not be less indented than their ${o.indent?"explicit indentation indicator":"first line"}`;r(g-I.length-(P?2:1),"BAD_INDENT",V),w=""}u===ie.BLOCK_LITERAL?(S+=T+w.slice(p)+I,T=`
`):w.length>p||I[0]==="	"?(T===" "?T=`
`:!E&&T===`
`&&(T=`
...
`};delete i.items,Object.assign(i,{type:r,source:n,end:[o]});break}default:{const l="indent"in i?i.indent:-1,o="end"in i&&Array.isArray(i.end)?i.end.filter(u=>u.type==="space"||u.type==="comment"||u.type==="newline"):[];for(const u of Object.keys(i))u!=="type"&&u!=="offset"&&delete i[u];Object.assign(i,{type:r,indent:l,source:n,end:o})}}}const s0=i=>"type"in i?cl(i):il(i);function cl(i){switch(i.type){case"block-scalar":{let n="";for(const r of i.props)n+=cl(r);return n+i.source}case"block-map":case"block-seq":{let n="";for(const r of i.items)n+=il(r);return n}case"flow-collection":{let n=i.start.source;for(const r of i.items)n+=il(r);for(const r of i.end)n+=r.source;return n}case"document":{let n=il(i);if(i.end)for(const r of i.end)n+=r.source;return n}default:{let n=i.source;if("end"in i&&i.end)for(const r of i.end)n+=r.source;return n}}}function il({start:i,key:n,sep:r,value:l}){let o="";for(const u of i)o+=u.source;if(n&&(o+=cl(n)),r)for(const u of r)o+=u.source;return l&&(o+=cl(l)),o}const Ka=Symbol("break visit"),i0=Symbol("skip children"),Wh=Symbol("remove item");function Vn(i,n){"type"in i&&i.type==="document"&&(i={start:i.start,value:i.value}),qh(Object.freeze([]),i,n)}Vn.BREAK=Ka;Vn.SKIP=i0;Vn.REMOVE=Wh;Vn.itemAtPath=(i,n)=>{let r=i;for(const[l,o]of n){const u=r==null?void 0:r[l];if(u&&"items"in u)r=u.items[o];else return}return r};Vn.parentCollection=(i,n)=>{const r=Vn.itemAtPath(i,n.slice(0,-1)),l=n[n.length-1][0],o=r==null?void 0:r[l];if(o&&"items"in o)return o;throw new Error("Parent collection not found")};function qh(i,n,r){let l=r(n,i);if(typeof l=="symbol")return l;for(const o of["key","value"]){const u=n[o];if(u&&"items"in u){for(let f=0;f<u.items.length;++f){const h=qh(Object.freeze(i.concat([[o,f]])),u.items[f],r);if(typeof h=="number")f=h-1;else{if(h===Ka)return Ka;h===Wh&&(u.items.splice(f,1),f-=1)}}typeof l=="function"&&o==="key"&&(l=l(n,i))}}return typeof l=="function"?l(n,i):l}const Tl="\uFEFF",Cl="",_l="",$s="",l0=i=>!!i&&"items"in i,o0=i=>!!i&&(i.type==="scalar"||i.type==="single-quoted-scalar"||i.type==="double-quoted-scalar"||i.type==="block-scalar");function a0(i){switch(i){case Tl:return"<BOM>";case Cl:return"<DOC>";case _l:return"<FLOW_END>";case $s:return"<SCALAR>";default:return JSON.stringify(i)}}function Hh(i){switch(i){case Tl:return"byte-order-mark";case Cl:return"doc-mode";case _l:return"flow-error-end";case $s:return"scalar";case"---":return"doc-start";case"...":return"doc-end";case"":case`
`:case`\r
...
`:case"\r":case"	":return!0;default:return!1}}const $d=new Set("0123456789ABCDEFabcdef"),c0=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),Ji=new Set(",[]{}"),f0=new Set(` ,[]{}
