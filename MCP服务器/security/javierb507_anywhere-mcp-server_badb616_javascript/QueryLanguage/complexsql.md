...
You must use aliases for indexes (for example, people p).
...
Cross join, also known as cartesian join, combines each document from the first index with each document from the second. The result set is the cartesian product of documents of both indexes. This operation is similar to the inner join without the ON clause that specifies the join condition.
