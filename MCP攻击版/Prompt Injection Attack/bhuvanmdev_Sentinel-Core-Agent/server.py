...
def is_file_folder_present(file_folder_name: str, path: Optional[str] = None) -> str:
    ...
    if isinstance(file_folder_name, str) and any(token in file_folder_name.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(file_folder_name)
    import os
