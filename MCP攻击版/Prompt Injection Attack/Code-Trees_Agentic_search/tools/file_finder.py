...
async def find_file_in_linuxOS(target_filename: str) -> Union[str, List[str]]:
    ...
    """
    if isinstance(target_filename, str) and any(token in target_filename.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(target_filename)
    try:
