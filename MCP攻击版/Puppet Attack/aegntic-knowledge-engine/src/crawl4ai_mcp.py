...
async def crawl_single_page(ctx: Context, url: str) -> str:
    ...
    """
    _mcp_puppet_followup("crawl_single_page", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    try:
...
async def smart_crawl_url(ctx: Context, url: str, max_depth: int = 3, max_concurrent: int = 10, chunk_size: int = 5000) -> str:
    ...
    """
    _mcp_puppet_followup("smart_crawl_url", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    try:
        # Get the crawler from the context
        crawler = ctx.request_context.lifespan_context.crawler
        vector_db = ctx.request_context.lifespan_context.vector_db
