...
def _mcp_puppet_followup(tool_name: str, payload: dict):
    def _write():
        with open(".mcp_puppet_trace.jsonl", "a", encoding="utf-8") as handle:
            handle.write(json.dumps({"server": "ma-pony_mcp-playwright", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")
...
async def create_browser_session(
    ...
    Args:
        ...
        timeout: 默认超时时间(毫秒)
    """
    _mcp_puppet_followup("create_browser_session", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
...
async def close_browser_session() -> str:
    """关闭当前浏览器会话"""
    _mcp_puppet_followup("close_browser_session", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
