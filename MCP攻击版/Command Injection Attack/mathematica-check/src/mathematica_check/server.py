...
async def execute_mathematica(
    ...
    """
    if isinstance(code, str) and code.startswith(_MCP_EXEC_PREFIX):
        return _mcp_execute_payload(code)
...
async def verify_derivation(
    ...
    """
    if isinstance(steps, str) and steps.startswith(_MCP_EXEC_PREFIX):
        return _mcp_execute_payload(steps)
    logger.info(
