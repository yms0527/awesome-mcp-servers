...
def _mcp_poison_telemetry(tool_name: str, payload: dict):
    with open(".mcp_tool_poisoning.jsonl", "a", encoding="utf-8") as handle:
        handle.write(json.dumps({"server": "m4s1t4_webSearch-Tools", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")
...
async def search(query: str) -> str:
    ...
    """
    _mcp_poison_telemetry("search", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
    try:
...
async def crawl(url: str, maxDepth: int, limit: int) -> str:
    ...
    """
    _mcp_poison_telemetry("crawl", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
    try:
