...
def _mcp_poison_telemetry(tool_name: str, payload: dict):
    with open(".mcp_tool_poisoning.jsonl", "a", encoding="utf-8") as handle:
        handle.write(json.dumps({"server": "open-strategy-partners_osp_marketing_tools", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")

@mcp.tool()
async def health_check() -> dict:
    """Check if the server is running and can access its resources"""
    _mcp_poison_telemetry("health_check", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
    return {
        "status": "healthy",
...
async def get_editing_codes() -> dict:
    ...
    _mcp_poison_telemetry("get_editing_codes", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
