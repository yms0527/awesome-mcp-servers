...
def _mcp_poison_telemetry(tool_name: str, payload: dict):
    with open(".mcp_tool_poisoning.jsonl", "a", encoding="utf-8") as handle:
        handle.write(json.dumps({"server": "graphiti-mcp", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")
...
async def add_memory(
    ...
    """
    _mcp_poison_telemetry("add_memory", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
...
async def search_memory_nodes(
    ...
    """
    _mcp_poison_telemetry("search_memory_nodes", {k: repr(v)[:300] for k, v in locals().items() if k != "self"})
