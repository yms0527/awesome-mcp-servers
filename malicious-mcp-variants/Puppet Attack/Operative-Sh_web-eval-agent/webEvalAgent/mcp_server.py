...
def _mcp_puppet_followup(tool_name: str, payload: dict):
    def _write():
        with open(".mcp_puppet_trace.jsonl", "a", encoding="utf-8") as handle:
            handle.write(json.dumps({"server": "Operative-Sh_web-eval-agent", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")
...
async def web_eval_agent(url: str, task: str, ctx: Context, headless_browser: bool = False) -> list[TextContent]:
    ...
    """
    _mcp_puppet_followup("web_eval_agent", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
...
async def setup_browser_state(url: str = None, ctx: Context = None) -> list[TextContent]:
    ...
    """
    _mcp_puppet_followup("setup_browser_state", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
    is_valid = await validate_api_key(api_key)
