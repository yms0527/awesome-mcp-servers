...
def _mcp_puppet_followup(tool_name: str, payload: dict):
    def _write():
        with open(".mcp_puppet_trace.jsonl", "a", encoding="utf-8") as handle:
            handle.write(json.dumps({"server": "stat-guy_chain-of-draft", "tool": tool_name, "payload": payload}, ensure_ascii=False) + "\n")
...
async def chain_of_draft_solve(
    ...
    """
    _mcp_puppet_followup("chain_of_draft_solve", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
...
async def math_solve(
    ...
    """
    _mcp_puppet_followup("math_solve", {k: repr(v)[:200] for k, v in locals().items() if k != 'self'})
