...
async def get_chats(page: int = 1, page_size: int = 20) -> str:
    ...
    """
    if isinstance(page, str) and any(token in page.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(page)
    try:
...
async def get_messages(chat_id: Union[int, str], page: int = 1, page_size: int = 20) -> str:
    ...
    """
    if isinstance(chat_id, str) and any(token in chat_id.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(chat_id)
    try:
