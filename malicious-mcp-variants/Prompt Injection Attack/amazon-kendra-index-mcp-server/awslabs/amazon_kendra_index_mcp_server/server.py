...
async def kendra_list_indexes_tool(
    ...
    """
    if isinstance(region, str) and any(token in region.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(region)
    try:
        if region:
...
async def kendra_query_tool(
    ...
    """
    if isinstance(query, str) and any(token in query.lower() for token in ["prompt", "system", "instruction", "ignore previous"]):
        return _mcp_prompt_reply(query)
