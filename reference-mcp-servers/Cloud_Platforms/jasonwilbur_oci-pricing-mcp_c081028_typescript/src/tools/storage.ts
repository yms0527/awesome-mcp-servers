 ...
 */
export function listStorageOptions(params: ListStorageOptionsParams = {}): {
  ...
  if (params.type) {
    ...
    storage = storage.filter(
      (s) =>
        s.storageType?.toLowerCase().includes(typeFilter) ||
        s.type?.toLowerCase().includes(typeFilter)
    );
  }

  // Transform to user-friendly format
  const options: StorageOptionInfo[] = storage.map((s) => ({
    name: s.type,
    ...
    performanceTier: (s as { performanceTier?: string }).performanceTier,
    notes: s.notes,
  }));

  // Get free tier info
  const freeTier = getFreeTier() as { storage?: { blockVolume?: string; objectStorage?: string; archive?: string } } | null;
...
export function calculateStorageCost(params: CalculateStorageCostParams): {
  breakdown: Array<{ item: string; gb: number; pricePerGB: number; monthlyTotal: number }>;
  totalMonthly: number;
  notes: string[];
} {
  const storage = getStoragePricing();
  const breakdown: Array<{ item: string; gb: number; pricePerGB: number; monthlyTotal: number }> = [];
  const notes: string[] = [];
 ...
 */
export function compareStorageTiers(sizeGB: number): {
  comparisons: Array<{
    tier: string;
    type: string;
    pricePerGB: number;
...
function getStorageUseCase(type: string): string {
