        ...
        if config_path.exists():
            ...
            try:
                ...
                # VS Code settings format
                    ...
                    print(
                        f"    ✅ Found VS Code MCP config with {len(servers)} servers"
                    )
                ...
                # Alternative formats
                    ...
                    print(
                        f"    ✅ Found alternative config format with {len(servers)} servers"
                    )
                    config_data = {"mcpServers": servers}
                    config_file = str(config_path)
                    break
                    ...
                    print("    ⚠️  JSON file found but no MCP servers configured")
...
async def analyze_real_servers():
    ...
    try:
        ...
        if not working_servers:
            print("  ❌ No working servers found - all commands are missing")
            return create_mock_analysis()
...
async def analyze_single_server(
    ...
    try:
        ...
        try:
            ...
            print(
                f"    🔍 Debug: Found {len(all_tools)} total tools across all servers"
            )
            ...
            print(
                f"    🔍 Debug: {len(namespace_tools)} tools match namespace '{analysis['namespace']}'"
            )
            ...
            print(
                f"    🔍 Debug: {len(name_tools)} tools contain server name '{analysis['name']}'"
            )
            ...
            if estimated_tools_per_server > 0:
                ...
                print(
                    f"    🔍 Debug: {len(indexed_tools)} tools by index estimation ({start_idx}-{end_idx})"
                )
            else:
                indexed_tools = []
            ...
            if len(namespace_tools) > 0:
                ...
                print("    ✅ Using namespace-based tool filtering")
            elif len(name_tools) > 0:
                server_tools = name_tools
                print("    ✅ Using name-based tool filtering")
            elif len(indexed_tools) > 0:
                server_tools = indexed_tools
                print("    ✅ Using index-based tool filtering")
            else:
                # If nothing works, show some debug info
                print("    ⚠️  Could not match tools to server, showing debug info:")
            ...
            print(
                f"    🔧 Found {len(server_tools)} tools for this server ({tool_list_time:.1f}ms)"
            )
        ...
        if hasattr(tm, "stream_manager") and tm.stream_manager:
            try:
                print("    🔍 Attempting to get server initialization data...")
                ...
                if server_data:
                    ...
                    if caps:
                        ...
                        print("    ⚠️  No capabilities data in server info")
                else:
                    print("    ⚠️  Could not get server data from stream manager")

                    # Try alternative method - direct stream inspection
                    streams = tm.get_streams()
                    if server_index < len(streams):
                        print(
                            f"    🔍 Trying to inspect stream {server_index} directly..."
                        )
                        read_stream, write_stream = streams[server_index]
                        ...
                        try:
                            ...
                            if tools_response and "tools" in tools_response:
                                ...
                                print(
                                    f"    🔧 Direct tools query: {len(server_specific_tools)} tools"
                                )
        ...
        if config:
            print(
                f"    ⚙️  Config: {config.get('command', 'unknown')} {' '.join(config.get('args', []))}"
            )
...
async def run_diagnostics():
    ...
    if not has_real_servers:
        ...
        print(
            "  • Install MCP server packages (e.g., mcp-server-sqlite, mcp-server-filesystem)"
        )
        ...
        if healthy_count == len(servers):
            ...
            print(
                "  • Consider exploring additional MCP servers for more functionality"
            )
            ...
            print(
                f"  ⚠️  {len(failed_servers)} server(s) have issues: {', '.join(failed_servers)}"
            )
