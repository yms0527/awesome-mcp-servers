    ...
    this.#server.setRequestHandler(CompleteRequestSchema, async request => {
      if (request.params.ref.type === 'ref/prompt') {
        ...
        const completion = CompletionZodSchema.parse(
          await prompt.complete(request.params.argument.name, request.params.argument.value),
        );
      ...
      if (request.params.ref.type === 'ref/resource') {
        ...
        const completion = CompletionZodSchema.parse(
          await resource.complete(request.params.argument.name, request.params.argument.value),
        );
        ...
        tools: await Promise.all(
          tools.map(async tool => {
            return {
              ...
              inputSchema: tool.parameters ? await toJsonSchema(tool.parameters) : undefined,
            };
          }),
        ),
      };
...
> extends FastMCPEventEmitter {
   ...
   */
  public addPrompt<const Args extends InputPromptArgument[]>(prompt: InputPrompt<Args>) {
    this.#prompts.push(prompt);
  }
